    ...
    <template slot="table-row" slot-scope="props">
        ...
        <span v-else-if="props.column.field === 'status.name'">
            <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full whitespace-nowrap" 
                :class="props.row.status.id == 8 ? 'bg-blue-100 text-blue-800' : 
                (props.row.status.id == 9 ? 'bg-yellow-100 text-yellow-800' : 
                (props.row.status.id == 11 ? 'bg-yellow-300 text-yellow-800' :'bg-green-100 text-green-800'))">
    ...
    methods: {
        removeItem(item) {
            this.$swal({
                ...
                if (result.value) {
                this.$axios.delete('/dashboard/boxes/' + item.id)
                .then(res => {
                    this.boxes = this.boxes.splice(this.boxes.findIndex(el => el.id === item.id), 1)
                })
                }
            })
        }
    },
    props: ['boxes', 'loading', 'isAction']
}
</script>
