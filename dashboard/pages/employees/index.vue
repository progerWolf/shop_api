  ...
  <div class="px-6">
    <div class="w-full py-8">
      <div class="flex items-center justify-between mb-10">
        <div class="">
          <h2 class="md:text-4xl text-2xl font-bold text-gray-700 mb-2">Сотрудники</h2>
          <span class="text-gray-800">Список сотрудников.</span>
        </div>
        <div class="flex">
          <nuxt-link to="/employees/create" class="d-btn d-btn-primary">
      ...
      <div>
        <vue-good-table
          ...
          <template slot="table-row" slot-scope="props">
            <span class="flex justify-around" v-if="props.column.field == 'action'">
                <nuxt-link :to="`/employees/${props.row.id}`"
                  class="text-blue-500" title="Просмотреть">
                  <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </nuxt-link>
                <nuxt-link :to="`/employees/edit/${props.row.id}`"
  ...
  methods: {
    ...
    removeItem(item) {
      this.$swal({
        ...
        if (result.value) {
          this.$axios.delete('/dashboard/employees/' + item.id)
          .then(res => {
            this.init()
          })
        }
