  ...
  <div class="px-6">
    <div class="w-full py-8">
      <div class="flex items-center justify-between w-full">
        <div class="">
          <h2 class="md:text-4xl text-2xl font-bold text-gray-700 mb-2.5">Разрешение</h2>
          <span class="text-gray-800">Список ролей.</span>
        </div>
        <div class="flex">
          <nuxt-link to="/roles/create" class="d-btn d-btn-primary">
        ...
        <vue-good-table
          ...
          <template slot="table-row" slot-scope="props">
            <span class="flex justify-center space-x-3" v-if="props.column.field == 'action'">
                <nuxt-link :to="`/roles/edit/${props.row.id}`"
  ...
  methods: {
    ...
    removeItem(item) {
      this.$swal({
        ...
        if (result.value) {
          this.$axios.delete(`/dashboard/roles/${item.id}`)
          .then(res => {
            this.init()
          })
        }
