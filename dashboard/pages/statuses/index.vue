  ...
  <div class="px-6">
    <div class="w-full py-8">
      <div class="flex items-center justify-between w-full">
        <div class="">
          <h2 class="md:text-4xl text-2xl font-bold text-gray-700">Статусы</h2>
          <!-- <span class="text-gray-800">Список пользователей.</span> -->
        </div>
        <div class="flex">
          <nuxt-link to="/statuses/create" class="d-btn d-btn-primary">
          ...
          <template slot="table-row" slot-scope="props">
            <span class="flex justify-center w-full" v-if="props.column.field == 'action'">
              ...
              <span v-else-if="props.column.field == 'created_at'">
                   <span class="text-sm px-3 py-1 rounded-full bg-green-300 text-gray-50">{{ new Date(props.row.created_at).toLocaleString() }}</span>
              </span>
          </template>
...
export default {
  ...
  methods: {
    init() {
      this.loading = true
      this.$axios.get('/dashboard/statuses')
      .then(res=> {
        this.rows = res.data.statuses
        this.loading = false
      })
    },
    removeItem(item) {
      this.$swal({
        ...
        this.$axios.delete('/dashboard/statuses/' + item.id).then(res => {
          this.init()
        })
      })
    }
