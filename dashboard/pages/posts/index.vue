  ...
  <div class="px-6">
    <div class="w-full py-8">
      <div class="flex items-center justify-between w-full">
        <div class="">
          <h2 class="md:text-4xl text-2xl font-bold text-gray-700 mb-2.5">Записи</h2>
          <span class="text-gray-800">Список записей.</span>
        </div>
        <div class="flex">
          <nuxt-link to="/posts/create" class="d-btn d-btn-primary">
        ...
        <vue-good-table
          ...
          <template slot="table-row" slot-scope="props">
            <span class="flex justify-center space-x-5" v-if="props.column.field == 'action'">
                <nuxt-link :to="`/posts/edit/${props.row.id}`"
...
export default {
  ...
  methods: {
    init() {
      this.loading = true
      this.$axios.get('/dashboard/posts')
      .then(res=> {
        this.rows = res.data.posts
        this.loading = false
      })
    },
    removeItem(item) {
      this.$swal({
        ...
        this.$axios.delete('/dashboard/posts/' + item.id).then(res => {
          this.init()
        })
      })
    }
